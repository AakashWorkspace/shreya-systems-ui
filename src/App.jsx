    ...
    <div className="flex h-screen overflow-hidden bg-ink-950">
      ...
      <aside className={`flex flex-col flex-shrink-0 transition-all duration-300 ease-in-out
        ...
        <div className={`flex items-center gap-3 px-4 h-16 border-b border-ink-700 overflow-hidden`}>
          ...
          {sidebarOpen && (
            <div className="overflow-hidden">
              <p className="text-gold-500 font-display font-bold text-sm leading-tight whitespace-nowrap">
                SHREYA SYSTEMS
              </p>
              <p className="text-gray-400 text-[10px] whitespace-nowrap tracking-wide">
                QUOTATION STUDIO
              </p>
            </div>
          )}
            ...
            return (
              <button
                ...
                className={`w-full flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all duration-150 text-left group
                  ${isActive
                    ? 'bg-gold-400/10 text-gold-600 border border-gold-400/30'
                    : 'text-gray-500 hover:text-gray-700 hover:bg-ink-800'}`}
              >
                <Icon className={`w-4 h-4 flex-shrink-0 ${isActive ? 'text-gold-500' : 'text-gray-400 group-hover:text-gray-600'}`} />
                {sidebarOpen && (
                  <div className="flex-1 min-w-0">
                    <p className={`text-sm font-medium leading-tight ${isActive ? 'text-gold-600' : 'text-gray-700'}`}>
                      {label}
                    </p>
                    <p className={`text-[10px] truncate ${isActive ? 'text-gold-500/70' : 'text-gray-400'}`}>
                      {desc}
                    </p>
                  </div>
                )}
        ...
        <button
          ...
          className="absolute -right-3 top-[72px] w-6 h-6 bg-white border border-ink-700
                     rounded-full flex items-center justify-center text-gray-400
                     hover:text-gold-500 hover:border-gold-400/50 transition-colors z-10 shadow-sm"
        >
          {sidebarOpen
            ? <X className="w-3 h-3" />
      ...
      <main className="flex-1 flex flex-col overflow-hidden">
        {/* Topbar */}
        <header className="h-16 flex items-center justify-between px-6 border-b border-ink-700 bg-white/80 backdrop-blur-sm flex-shrink-0">
          <div>
            <h1 className="font-display text-lg text-gray-900 leading-tight">
              {currentNav?.label}
            </h1>
